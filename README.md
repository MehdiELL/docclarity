...
### AI documentation-clarity rater on GenLayer
...
**Deployed contract (Testnet Bradbury):** [`0x0B74651e249f7Ac9072Aea1e1d0E5e90DD248A5c`](https://explorer-bradbury.genlayer.com/address/0x0B74651e249f7Ac9072Aea1e1d0E5e90DD248A5c)
...
2. **`evaluate(id)`** - each validator fetches the source, classifies the content
   (one of: clear, adequate, confusing) and scores it 0-100, then must **agree** on the label and a similar
   score via the Equivalence Principle.
3. The **label, score, and reasoning** are written on chain.

## The Intelligent Contract

`contracts/docclarity.py` targets the GenVM Python runner (pinned by hash):

- **Integers only** - the clarity score is `u8` (0-100); no floats.
...
genvm-lint check contracts/docclarity.py
