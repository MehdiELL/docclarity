# Direct-mode unit tests for DocClarity.  Run: pytest tests/direct/ -v
import json

CONTRACT = "contracts/docclarity.py"
...
def test_submit_and_evaluate(direct_vm, direct_deploy, direct_alice):
    ...
    direct_vm.mock_llm(r".*JSON.*", json.dumps({"label": "clear", "score": 80, "reasoning": "ok"}))
    c.evaluate(0)
    item = c.get_item(0)
    assert item["label"] == "clear"
